...
# No live plotting during processing. A single summary plot is created at the end.
ENABLE_FINAL_PLOT  = True
FINAL_PLOT_FILE    = "eeg_summary_plot.png"
...
def _store_history(current_t: float, smooth_feats: dict, current_state: str):
    """Store one processed frame for the final plot."""
    global _hist_n
...
def _show_final_plot(wall_time_seconds: float, processing_time_seconds: float):
    """Create one plot after all processing has finished."""
    if not ENABLE_FINAL_PLOT:
        return

    if _hist_n <= 0:
        print("No post-calibration history available; final plot skipped.")
        return

    plot_start = time.perf_counter()

    t = _hist["times"][:min(_hist_n, _MAX_HISTORY)]
    engagement = _hist["engagement"][:min(_hist_n, _MAX_HISTORY)]
    relaxation = _hist["relaxation"][:min(_hist_n, _MAX_HISTORY)]
    states = _hist["state"][:min(_hist_n, _MAX_HISTORY)]
    ...
    ax_metrics.plot(t, engagement, label="Engagement",
                    color=STATE_COLORS["FOCUSED"])
    ax_metrics.plot(t, relaxation, label="Relaxation",
                    color=STATE_COLORS["DRIFTING"])

    _add_event_lines(ax_metrics)

    final_state = states[-1] if len(states) else "unknown"
    ax_metrics.set_title(
        "Final EEG metrics"
        f" | final state: {final_state}"
        f" | processing: {processing_time_seconds:.3f}s"
        f" | total: {wall_time_seconds:.3f}s",
        fontweight="bold"
    )
    ax_metrics.set_ylabel("Metric value")
    ax_metrics.legend(fontsize=8, loc="upper right")

    for t_s, t_e, st in _rle_spans(t, states):
    ...
    fig.tight_layout()
    fig.savefig(FINAL_PLOT_FILE, dpi=150)
    print(f"Final plot saved to: {FINAL_PLOT_FILE}")
    print(f"Plot generation time: {time.perf_counter() - plot_start:.3f}s")

    plt.show()


async def processing_loop():
    """Process incoming samples without live plotting. The plot is created once at the end."""
    global _dirty, _start_time, _processing_time_seconds
    ...
    while live_buffer.is_running:
        ...
        if len(filtered) < nperseg:
            _processing_time_seconds += time.perf_counter() - compute_start
            await asyncio.sleep(0)
            continue
        ...
        if not calibration.complete:
            ...
            _store_history(current_t, smooth_feats, current_state)
...
async def main():
    global _start_time, _hist_n, _dirty, _processing_time_seconds
    ...
    try:
        ...
        print(f"Total wall-clock time: {wall_time_seconds:.3f}s")
        print(f"Processing time only:  {_processing_time_seconds:.3f}s")
        _show_final_plot(wall_time_seconds, _processing_time_seconds)
